...
const TimeSelectModal = ({ handleClose }: HandleClose) => {
	...
	const [time, setTime] = useState<any>(
		dayjs(
			`2023-09-01T${sessionStorage.time?.split(' ')[0]}:${sessionStorage.time?.split(' ')[1]}`
		)
	);
			...
			<Box sx={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
				...
				>
					<ToggleButtonGroup
						...
						exclusive
					>
						<ToggleButton value="departure" onClick={handleClick}>
							Depart at
						</ToggleButton>
						<ToggleButton value="arrival" onClick={handleClick}>
							Arrive by
						</ToggleButton>
						<ToggleButton value="now" onClick={handleDepartNow}>
							Depart Now
						</ToggleButton>
					</ToggleButtonGroup>
