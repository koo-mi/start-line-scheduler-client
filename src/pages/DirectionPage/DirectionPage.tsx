	...
	useEffect(() => {
		async function getDirectionData(): Promise<void> {
			...
			const dirRes = await axios.get(
				`${URL}/direction/${sessionStorage.start}/${sessionStorage.end}/${sessionStorage.time}/${sessionStorage.mode}/${sessionStorage.type}`,
				{
					headers: {
		...
		<Container component="main" id="main-container" sx={{ mb: '4.5rem' }}>
			...
			<Box className="direction-detail__header" sx={{ pb: 0 }}>
				...
				<AddLocationAltOutlinedIcon
					className="direction-detail__location-icon"
					fontSize="large"
					onClick={() => {
			...
			>
				...
				<div className="direction-detail__info-box">
					<div className="direction-detail__time-cont">
						<div>
							<p className="direction-detail__time-text">
								{directionData!.departureTime} - {directionData!.arrivalTime}
							</p>
							<p className="direction-detail__dd-text">
						...
						{directionData!.stepsSummary.map((step, i) => {
							...
							return (
								<div key={i} className="direction-detail__inst">
									...
									<p className="direction-detail__inst-duration">
										{step.duration}
									</p>
								</div>
							);
